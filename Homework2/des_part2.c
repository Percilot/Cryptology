int des_init(int mode)
{
   ...
   kn = (unsigned char(*)[8])malloc(sizeof(char) * 256 * 8);
   if (kn == NULL)
   {
...
static void perm_init(char perm[8][256][8], char p[64]) /* 64-bit, either init or final */
{
   ...
   for (i = 0; i < 8; i++) /* [%] i=SectionNum, each section contains 4 bits; */ /* each input nibble position */
   {
      for (j = 0; j < 256; j++) /* [%] j=SectionValue varying from 0 to 15; */ /* each possible input nibble */
      {
